...
![configurations](https://cdn.nlark.com/yuque/0/2020/png/1354552/1602845382065-5c41abf5-ee10-4c93-89e4-2a9b831c3ceb.png)
...
1. 根据当前请求，找到**HandlerExecutionChain**，它包含可以处理请求的`handler`以及该`handler`的所有拦截器。

2. 按**顺序执行**所有拦截器的`preHandle`方法：
    - 如果当前拦截器的`preHandle`方法返回`true`，则继续执行下一个拦截器的`preHandle`方法。
    - 如果当前拦截器的`preHandle`方法返回`false`，则：
        - 停止执行后续拦截器的`preHandle`方法。
        - **倒序执行**所有已经执行过的拦截器的`afterCompletion`方法。
        - 不执行目标方法。

3. 如果所有拦截器的`preHandle`方法都返回`true`，则执行目标方法。

4. 在目标方法执行完成后，按**倒序执行**所有拦截器的`postHandle`方法。

5. 如果在执行目标方法或拦截器的任何步骤中发生异常，则会直接**倒序触发**所有拦截器的`afterCompletion`方法。

6. 页面渲染完成后，无论是否发生异常，都会**倒序触发**所有拦截器的`afterCompletion`方法。

**补充说明：**

- `preHandle`：在目标方法执行之前调用，用于拦截和预处理请求。
- `postHandle`：在目标方法执行完成后调用，但在视图渲染之前调用，用于对模型数据进行修改或其他处理。
- `afterCompletion`：在视图渲染完成后调用，用于资源清理或记录日志等操作。
- 拦截器的执行顺序由其注册顺序决定，先注册的拦截器先执行`preHandle`，后执行`postHandle`和`afterCompletion`。
